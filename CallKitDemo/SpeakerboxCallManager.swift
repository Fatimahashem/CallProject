...
final class SpeakerboxCallManager: NSObject {
    ...
    func setMute(call: SpeakerboxCall, muted: Bool) {
        let setMutedCCallAction = CXSetMutedCallAction(call: call.uuid, muted: muted)
        let transaction = CXTransaction()
        transaction.addAction(setMutedCCallAction)

        requestTransaction(transaction, action: Call.mute.rawValue)
    }
